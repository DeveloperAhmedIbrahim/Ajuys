...
if ($pt->second_page == 'about') {
        $user_data->education   = $db->where('user_id', $user_data->id)->get(T_EDUCATION);
        $user_data->experience = $db->where('user_id', $user_data->id)->orderBy('id', 'ASC')->get(T_EXPERINCE);
}
...
$pt->custom_fields  = "";
$user_data->selected_education = "";
$user_data->selected_experience = "";

if($user_data->show_education){
    $educ = $db->where('id', $user_data->education_id)->getOne (T_EDUCATION);
    $user_data->selected_education = $educ->institude ;
}

if($user_data->show_experience){

    $expr = $db->where('id', $user_data->experience_id)->getOne (T_EXPERINCE);
    $user_data->selected_experience = $expr->company_name ;
}
